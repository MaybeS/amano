...
#ifndef AMANO_UTILITY_H
#define AMANO_UTILITY_H
...
void ConvertRays2Pixels(const Calibration& calib, const Mat& rays,
    ...
    Mat& pix_x = *pixels_x_ptr;
    Mat& pix_y = *pixels_y_ptr;
    const float fx = calib.fx, fy = calib.fy, cx = calib.cx, cy = calib.cy;
    ...
    if (calib.fisheye) {
        for (int y = 0; y < sz.height; ++y) {
            for (int x = 0; x < sz.width; ++x) {
                ...
                float rx = ray[0], ry = ray[1], rz = ray[2];  // Assume 3 channels.
...
void MakeCylinderRays(const Size& sz, const Vec3f& center, const Vec3f& right, float deg, Mat* rays) {
    ...
    const double rad = center.dot(front);
    const double dx = 2.0 / (w - 1), dr = 2.0 / (h - 1);
    // const double dr = 2.0 / (w - 1), dy = 2.0 / (h - 1);
    rays->create(sz, CV_32FC3);
    for (int y = 0; y < sz.height; ++y) {
        const double th = (y * dr - 1.0) * deg * d2r;
        const Vec3f ray_y = -sin(th) * up + cos(th) * front;
        ...
        // const Vec3f ray_y = center - front - (y * dy - 1.0) * up;
        // for (int x = 0; x < sz.width; ++x) {
        //     const double th = (x * dr - 1.0) * deg * d2r;
        //     const Vec3f r = NormalizeVec3f(ray_y + sin(th) * right + cos(th) * front);
        //     memcpy(rays->ptr(y, x), r.val, sizeof(float) * 3);
    }
}
