...
public:
    ...
    Warp(const Calibration& calib, const Size& output_size, int plane_mode)
    : calib(calib), output_size(output_size), plane_mode(plane_mode) {
        Reset();
        Update();
    }
    ...
    void DrawBoundingBox(Mat* output) const {
        ...
        }
        {
            ...
            for (int i = 0; i <= num_horizon_rays; ++i) {
                const double th = 2 * M_PI * i / num_horizon_rays;
    ...
    void Reset() {
        ...
        up << 0.0, 1.0, 0.0;
        right << 1.0, 0.0, 0.0;
        right *= double(output_size.width) / output_size.height;
    }
